...
var utmmedium = '[[UTM Medium|string]]';
...
function getUTMCampaign(session) {
    ...
    if (session.utmAttributes && session.utmAttributes.source && session.utmAttributes.medium === utmmedium) {
        return true;
    }
    var retval = false;
    ...
    session.events.forEach(event => {
       if (event.type === 'VIEW') {
           if (event.arbitraryData && event.arbitraryData.utm_source && event.arbitraryData.utm_medium === utmmedium) {
